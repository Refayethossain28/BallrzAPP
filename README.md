...
- **🗣️ [Lingua — the fluency engine](https://refayethossain28.github.io/BallrzAPP/lingua/)** — learn **every language in the world**: 180+ built in (listed alphabetically, all treated equally) plus a free-type option for anything else, with regional dialect/variety support for several languages. Built on the five research-backed fluency levers: an **SRS memory deck** (SM-2 spaced repetition), **pronunciation-scored speaking drills** (on-device), **AI-graded production drills**, **graded stories** at your level, and a **streak + daily XP** habit loop — all arranged into a **CEFR course (A1 → B2, 24 units)**. Runs offline with a starter phrasebook; enable **Live AI** (paste your own Anthropic key, run the local proxy [`lingua/server.mjs`](./lingua/server.mjs), or deploy the hosted function) for full accuracy. **Installable PWA**.
