...
- **⭐ [Imposter — the party game](https://refayethossain28.github.io/BallrzAPP/imposter/)** — the best **pass-and-play** social-deduction word game: one phone, **3–12 players**, no account. Everyone shares one secret **word** — everyone except the **imposter**, who has to bluff blind. Pass the phone so each player privately flips their card (a slick 3D reveal, optional **hold-to-reveal** for shoulder-surf safety), give a **one-word clue** in the dealt turn order, run the **discussion timer**, then the table agrees on one accusation and votes. Unmask the imposter and the crew scores — unless the caught imposter can **name the secret word** and *steal it at the buzzer* (+3). Mis-accuse and the imposter walks. **12 curated word packs** (Food, Animals, Places, Movies, Sports, Jobs, Nature, Tech, Travel, Music, Fantasy, Household) plus **Mixed**, **1–N imposters** (auto-capped so the crew always outnumbers them), and a **decoy mode** where the imposter secretly holds a *different* word from the same category. Sound effects, haptics, a persistent **scoreboard** across rounds. The deal / vote-tally / scoring core is a pure, deterministic, unit-tested engine ([`imposter/engine.js`](./imposter/engine.js), `npm run test:imposter`). Fully offline, **installable PWA**.
