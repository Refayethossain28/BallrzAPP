...
- **[RentMatch](https://refayethossain28.github.io/BallrzAPP/rentmatch.html)** — UK lettings marketplace. Landlords advertise property, renters search & enquire, both parties message and book a viewing, then sign a UK-compliant Assured Shorthold Tenancy. The landlord is charged £100 on signing. Toggle **Renter ↔ Landlord** in the header to play both sides — the whole deal lifecycle runs in one browser, offline. **Installable PWA** (Safari/Chrome → Add to Home Screen). The production architecture lives in [`rentmatch/`](./rentmatch/) and [`docs/rentmatch-foundation.md`](./docs/rentmatch-foundation.md).
