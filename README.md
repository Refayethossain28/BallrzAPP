...
- **🛰️ [Atlas — LAN node registry & scheduler](./atlas/)** — a local network control plane that lets **Hermes** route work to the best available machine. Each box runs a tiny zero-dependency **Node Agent** that heartbeats its specs (hostname, IP, CPU, RAM, GPU, VRAM), installed services, capabilities (`ocr`, `embeddings`, `local_llm`, `vision`, `whisper`, …), current load and health to a **Central Registry**. A **Scheduler** takes a task request, picks a healthy node that has the required capabilities (deliberately simple and *explainable* — capability match, then health, then lowest load), and dispatches through **Ray** (a pluggable adapter, stubbed in the MVP) before the result returns to Hermes. A live **dashboard** shows the whole fleet at a glance; missed heartbeats flip nodes to `stale` and out of scheduling, and every refusal comes with a reason (`503 "no healthy node has all required capabilities [whisper]"`) rather than a silent failure. Built *visible and reliable first*: the decision core ([`atlas/core.mjs`](./atlas/core.mjs)) is pure, deterministic and unit-tested (`npm run test:atlas`). Run `node atlas/registry.mjs` then point agents at it — see [`atlas/README.md`](./atlas/README.md).
...
Atlas LAN node registry & scheduler (the control plane that routes work to the best machine):

```sh
node atlas/registry.mjs                         # registry + scheduler + dashboard on :8795
# on each machine, declare what it can do and point it at the registry:
ATLAS_CAPS="ocr,vision,local_llm" ATLAS_REGISTRY=http://<host>:8795 node atlas/agent.mjs
# then open http://localhost:8795 for the live fleet dashboard
```
