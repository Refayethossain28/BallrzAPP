...
- **🪙 [BallrzCoin — my own cryptocurrency](https://refayethossain28.github.io/BallrzAPP/coin/)** — a complete **Bitcoin-style proof-of-work cryptocurrency built from raw bytes up, with zero dependencies**: SHA-256 implemented from FIPS 180-4, ECDSA on **secp256k1** (Bitcoin's exact curve, with RFC 6979 deterministic signatures and compressed public keys), base58check addresses, a **UTXO ledger**, merkle trees, **proof-of-work mining with Bitcoin-style difficulty retargeting**, a block reward **halving every 210 blocks** toward the scarcest hard cap ever minted — **only 21 BLZ will ever exist**, a million times scarcer than Bitcoin's 21,000,000 — transaction fees paid to miners, a fee-priority mempool, and **fork choice by cumulative work** (real Nakamoto consensus). Create wallets, **⛏ mine blocks live in your browser**, send coins and watch them confirm in the block explorer — then open a second tab: **each tab is an independent node**, gossiping blocks and transactions over `BroadcastChannel` and converging on the heaviest chain. The whole consensus core is a pure engine ([`coin/engine.js`](./coin/engine.js)) verified against published crypto test vectors (FIPS 180-4, RFC 4231, the RFC 6979 secp256k1 vector) and attacked directly in tests — double-spends, forged signatures, tampered blocks, coinbase inflation and doctored forks are all rejected (`npm run test:coin` — 30 tests). What's real vs. simplified is documented in [`coin/README.md`](./coin/README.md) — a teaching toy, not money.
