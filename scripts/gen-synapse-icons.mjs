#!/usr/bin/env node
/**
 * Generates the Synapse app icons as real PNGs — no image libraries, just
 * Node's built-in zlib (same minimal PNG encoder approach as
 * gen-magpie-icons.mjs). Rasterizes the same motif as synapse/icon.svg:
 * a radial-dark rounded square holding two neuron-coins — the violet
 * question node firing across the synaptic gap to the cyan answer node —
 * with the gold spark of the minted coin at the junction.
 *
 * Run: node scripts/gen-synapse-icons.mjs   (writes icon-180/192/512.png into synapse/)
 */
import zlib from 'node:zlib';
import { writeFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';

const OUT = join(dirname(fileURLToPath(import.meta.url)), '..', 'synapse');

/* ---- minimal PNG (RGBA, no palette) ---- */
const crcTable = (() => {
  const t = new Uint32Array(256);
  for (let n = 0; n < 256; n++) { let c = n; for (let k = 0; k < 8; k++) c = c & 1 ? 0xEDB88320 ^ (c >>> 1) : c >>> 1; t[n] = c >>> 0; }
  return t;
})();
function crc32(buf) { let c = 0xFFFFFFFF; for (let i = 0; i < buf.length; i++) c = crcTable[(c ^ buf[i]) & 0xff] ^ (c >>> 8); return (c ^ 0xFFFFFFFF) >>> 0; }
function chunk(type, data) {
  const len = Buffer.alloc(4); len.writeUInt32BE(data.length);
  const td = Buffer.concat([Buffer.from(type, 'latin1'), data]);
  const crc = Buffer.alloc(4); crc.writeUInt32BE(crc32(td));
  return Buffer.concat([len, td, crc]);
}
function encodePNG(N, rgba) {
  const sig = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]);
  const ihdr = Buffer.alloc(13);
  ihdr.writeUInt32BE(N, 0); ihdr.writeUInt32BE(N, 4); ihdr[8] = 8; ihdr[9] = 6; // 8-bit RGBA
  const stride = N * 4;
  const raw = Buffer.alloc((stride + 1) * N);
  for (let y = 0; y < N; y++) { raw[y * (stride + 1)] = 0; rgba.copy(raw, y * (stride + 1) + 1, y * stride, y * stride + stride); }
  const idat = zlib.deflateSync(raw, { level: 9 });
  return Buffer.concat([sig, chunk('IHDR', ihdr), chunk('IDAT', idat), chunk('IEND', Buffer.alloc(0))]);
}

/* ---- helpers ---- */
const lerp = (a, b, t) => a + (b - a) * t;
const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));
// the firing gradient: violet → cyan → gold, along x+y
function brandColor(t) {
  t = clamp(t, 0, 1);
  const stops = [[0x8f, 0x7c, 0xf7], [0x4d, 0xd8, 0xff], [0xe8, 0xb3, 0x4c]];
  const seg = t < 0.6 ? 0 : 1;
  const lt = seg === 0 ? t / 0.6 : (t - 0.6) / 0.4;
  return [0, 1, 2].map((i) => Math.round(lerp(stops[seg][i], stops[seg + 1][i], lt)));
}
function capsuleDist(px, py, ax, ay, bx, by) {
  const abx = bx - ax, aby = by - ay;
  const t = clamp(((px - ax) * abx + (py - ay) * aby) / (abx * abx + aby * aby), 0, 1);
  const dx = px - (ax + abx * t), dy = py - (ay + aby * t);
  return Math.hypot(dx, dy);
}
// distance to a 4-point sparkle (union of two thin diamonds)
function sparkDist(px, py, cx, cy, r, w) {
  const dx = Math.abs(px - cx), dy = Math.abs(py - cy);
  const d1 = dx / w + dy / r - 1;   // tall diamond
  const d2 = dx / r + dy / w - 1;   // wide diamond
  return Math.min(d1, d2) * Math.min(w, r) * 0.7; // approx px distance
}

function render(N) {
  const rgba = Buffer.alloc(N * N * 4);
  const S = N / 512; // design space is 512
  const corner = 116 * S;
  const aa = 1.0; // anti-alias feather in px

  // geometry from icon.svg
  const q = { x: 160 * S, y: 176 * S, r: 74 * S, w: 20 * S, col: [0x8f, 0x7c, 0xf7] };
  const a = { x: 352 * S, y: 336 * S, r: 74 * S, w: 20 * S, col: [0x4d, 0xd8, 0xff] };
  const AXONS = [
    [212, 220, 268, 266],
    [292, 286, 300, 292],
  ];
  const axonW = 9 * S;

  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      const i = (y * N + x) * 4;
      // rounded-square mask
      const qx = Math.abs(x - N / 2) - (N / 2 - corner);
      const qy = Math.abs(y - N / 2) - (N / 2 - corner);
      const rd = Math.hypot(Math.max(qx, 0), Math.max(qy, 0)) - corner;
      const mask = clamp(0.5 - rd / aa, 0, 1);
      if (mask <= 0) { rgba[i + 3] = 0; continue; }

      // radial-dark background, violet-leaning (light falls from the upper middle)
      const dTop = Math.hypot(x - N * 0.5, y - N * 0.18) / (N * 0.9);
      let r = Math.round(lerp(0x17, 0x0a, clamp(dTop, 0, 1)));
      let g = Math.round(lerp(0x12, 0x0d, clamp(dTop, 0, 1)));
      let b = Math.round(lerp(0x36, 0x16, clamp(dTop, 0, 1)));

      const grad = clamp((x + y) / (2 * N), 0, 1);
      const [gr, gg, gb] = brandColor(grad);

      // the two neuron-coins
      for (const node of [q, a]) {
        const d = Math.abs(Math.hypot(x - node.x, y - node.y) - node.r) - node.w / 2;
        const al = clamp(0.5 - d / aa, 0, 1);
        if (al > 0) {
          r = Math.round(lerp(r, node.col[0], al));
          g = Math.round(lerp(g, node.col[1], al));
          b = Math.round(lerp(b, node.col[2], al));
        }
      }

      // the firing axon segments, gradient-colored
      for (const [ax, ay, bx2, by2] of AXONS) {
        const d = capsuleDist(x, y, ax * S, ay * S, bx2 * S, by2 * S) - axonW;
        const al = clamp(0.5 - d / aa, 0, 1);
        if (al > 0) { r = Math.round(lerp(r, gr, al)); g = Math.round(lerp(g, gg, al)); b = Math.round(lerp(b, gb, al)); }
      }

      // the minted coin: a gold spark in the synaptic gap
      const dS = sparkDist(x, y, 282 * S, 272 * S, 38 * S, 11 * S);
      const aS = clamp(0.5 - dS / aa, 0, 1);
      if (aS > 0) { r = Math.round(lerp(r, 0xe8, aS)); g = Math.round(lerp(g, 0xb3, aS)); b = Math.round(lerp(b, 0x4c, aS)); }

      rgba[i] = r; rgba[i + 1] = g; rgba[i + 2] = b; rgba[i + 3] = Math.round(mask * 255);
    }
  }
  return encodePNG(N, rgba);
}

for (const n of [180, 192, 512]) {
  writeFileSync(join(OUT, `icon-${n}.png`), render(n));
  console.log(`synapse/icon-${n}.png`);
}
