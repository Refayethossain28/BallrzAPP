#!/usr/bin/env node
/**
 * Generates the Rig app icons as real PNGs — no image libraries, just Node's
 * built-in zlib (same minimal PNG encoder as gen-miner-icons.mjs). A dark
 * rounded tile with a warm glow, an amber coin ring and an amber pickaxe —
 * the same motif as rig/icon.svg.
 *
 * Run: node scripts/gen-rig-icons.mjs   (writes icon-180/192/512.png into rig/)
 */
...
const OUT = join(dirname(fileURLToPath(import.meta.url)), '..', 'rig');
...
// pickaxe geometry (512-unit space, same as rig/icon.svg)
...
function render(N) {
  ...
  const px = 1;
  const AMBER_HI = [255, 207, 125], AMBER_LO = [196, 106, 16];
  ...
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      ...
      if (cardA > 0) {
        const gx = (x - 0.32 * N) / N, gy = (y - 0.24 * N) / N;
        const t = clamp01(Math.hypot(gx, gy) / 1.05);
        let c = t < 0.55 ? mix([42, 32, 19], [20, 16, 10], t / 0.55) : mix([20, 16, 10], [10, 8, 6], (t - 0.55) / 0.45);

        const dc = Math.hypot(x - N / 2, y - N / 2);
        const diag = clamp01((x + y) / (2 * N));
        const amberAt = mix(AMBER_HI, AMBER_LO, diag);
        const ringA = edge(Math.abs(dc - u(168)) - u(5), px);
        if (ringA > 0) c = mix(c, amberAt, ringA);
        ...
        if (pickA > 0) c = mix(c, amberAt, pickA);
