#!/usr/bin/env node
/**
 * Magpie companion fetch server — zero dependencies, Node built-ins only.
 *
 * The static Magpie page can only scrape its own origin (plus CORS-open
 * pages): the browser's same-origin policy is doing its job. This little
 * server is the honest fix — run it and open Magpie from it, and the scraper
 * can fetch ANY page on the internet:
 *
 *     node magpie/server.mjs            # then open http://localhost:8796
 *
 * What it does:
 *   - serves the magpie/ directory (so the page and /api/* share one origin)
 *   - GET /api/health          → {ok:true, magpie:"server"} (the page's probe)
 *   - GET /api/fetch?url=...   → fetches the page server-side and returns
 *                                {url, status, html} for the in-page engine
 *                                to parse and extract. HTML only, 1.5 MB cap,
 *                                10 s timeout, ≤5 redirects.
 *
 * Honesty & safety:
 *   - It identifies itself (User-Agent: MagpieBot) and RESPECTS robots.txt —
 *     the rules are fetched per-origin, cached, and evaluated with the same
 *     unit-tested parser the page uses (magpie/engine.js in a vm sandbox).
 ...
 *     MAGPIE_ALLOW_PRIVATE=1 to lift it for local development only.
 *   - It fetches on your behalf, from your machine, one page at a time with
 *     the page's polite delay — a prototype scraper, not a datacentre.
 */
...
vm.runInContext(readFileSync(join(DIR, 'engine.js'), 'utf8'), sandbox, { filename: 'magpie/engine.js' });
const Engine = sandbox.module.exports;

/* ---- SSRF guard (same posture as seeker/server.mjs and voyager/proxy.js) ---- */
...
async function apiFetch(rawUrl) {
  ...
  if (!url) return { status: 400, body: { error: 'not a scrapeable URL' } };
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (u.pathname === '/api/health') return sendJSON(res, 200, { ok: true, magpie: 'server' });
    ...
    // static: magpie/ only, no traversal
...
server.listen(PORT, HOST, () => {
  console.log(`🐦 Magpie fetch server → http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}`);
  console.log(`   robots.txt respected (UA: MagpieBot), SSRF guard ${ALLOW_PRIVATE ? 'OFF (private hosts allowed)' : 'on'}`);
});
